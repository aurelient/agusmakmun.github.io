...
__author__ = [
    ...
    'Aurelien Tabard <http://tabard.fr>'
]
...
"""
    ...
    def _format_author_list(immut_author_list):
        ...
        for author in immut_author_list:
            ...
            new_auth = new_auth[0] + " " + new_auth[1][0] + "."
    ...
    def _get_pub_str(pub, prefix, gidx):
        ...
        if (pub['ENTRYTYPE'] == 'inproceedings'):
            # assert('booktitle' in pub and 'pages' in pub and 'address' in pub and ('publisher' in pub or 'organization' in pub))
            # yearVenue = "{} {}".format(pub['_venue'], pub['year'])
            # venue = "In {} {}".format(pub['_venue'], pub['year'])
            venue = "In <i>{}</i>. (pp. {}).".format(pub['booktitle'], pub['pages'])
            if 'address' in pub:
                venue = venue + " {},".format(pub['address'])
            if 'publisher' in pub:
                venue = venue + " {}.".format(pub['publisher'].replace('\\newline','<br/>'))
            elif 'organization' in pub:
                venue = venue + " {}.".format(pub['organization'])
        ...
        #
        elif (pub['ENTRYTYPE'] == 'misc'):
            venue = "In <i>{}</i>.".format(pub['booktitle'])
            if 'pages' in pub:
                venue = venue + "pp. {}.".format(pub['pages'])
            if 'address' in pub:
                venue = venue + " {},".format(pub['address'])
            if 'publisher' in pub:
                venue = venue + " {}. ".format(pub['publisher'].replace('\\newline','<br/>'))
            ...
            venue = "{}. {}.".format(pub['number'], pub['institution'])

        else:
            # assert('_venue' in pub and 'year' in pub)
            venue = "In {} {}".format(pub['_venue'], pub['year'])

        imgPth = 'images/publications/{}.png'.format(pub['ID'])
        if os.path.isfile(imgPth):
            imgStr = '<img src="{}"/>'.format(imgPth)
        else:
            imgStr = '_'
...
""".format(pub['ID']))
        ...
        if 'doi' in pub:
            links.append(
                "[<a href=\'{}\' target='_blank'>doi</a>] ".format(pub['doi']))
        if 'codeurl' in pub:
            links.append(
...
'''.format(pub['ID'], abstract)
    ...
    '''.format(imgStr, author_str, pubYear, title, venue, links, abstract)
